    ...
    <>
      <div className="space-y-4">
        {Object.entries(groupedLogs).map(([monthYear, groupLogs]) => (
          <div key={monthYear}>
            <div className="sticky top-0 bg-slate-50 z-10 py-2 mb-3">
              <button
                onClick={() => toggleMonth(monthYear)}
                className="flex items-center gap-2 text-base font-bold text-slate-800 hover:text-slate-600 transition"
              >
                <span>{monthYear}</span>
                ...
                <span className="text-xs text-slate-500 font-normal">({groupLogs.length})</span>
              </button>
            </div>
            ...
            <div className="relative">
              ...
              <div className="space-y-3">
                ...
                <div key={log.id} className="relative pl-11">
                  <div className="absolute left-0 w-8 h-8 bg-slate-800 rounded-full flex items-center justify-center text-white font-bold shadow-lg text-xs">
                    {formatDate(log.event_date).split(' ')[1]}
                  </div>

                  <div
                    onClick={() => openEditModal(log)}
                    className="bg-white rounded-lg shadow-md hover:shadow-lg transition p-3 group cursor-pointer"
                  >
                    ...
                    {log.description && (
                      <p className="text-slate-600 mb-2 leading-relaxed text-xs">
                        {log.description}
                      </p>
                    )}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[60] p-4">
          <div className="bg-white rounded-xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-white border-b border-slate-200 p-4 flex items-center justify-between">
              <h2 className="text-lg font-bold text-slate-800">Edit Log</h2>
            ...
            <form onSubmit={handleEditSubmit} className="p-4 space-y-3">
              ...
              <div>
                <label className="block text-xs font-medium text-slate-700 mb-1">Trip Name</label>
                <input
                  type="text"
